...
class ImagesServiceTest {
  @Mock private IImagesRepository iImagesRepository;
  ...
  public void setUp() {
    ...
    imageGrandblue.setUrl(
        "https://static0.gamerantimages.com/wordpress/wp-content/uploads/2024/01/granblue-fantasy-relink-key-art.jpg");
  ...
  @Test
  void updateImageFavorite() {
    when(iImagesRepository.findById(2)).thenReturn(Optional.of(imageGrandblue));
    when(iImagesRepository.save(any(Images.class))).thenReturn(imageGrandblue);

    Images imageToUpdateFavorite = imagesService.updateFavorite(2);

    verify(iImagesRepository).findById(2);
    verify(iImagesRepository).save(any(Images.class));

    assert imageToUpdateFavorite.isFavorite();
  }
}
